...
resolve,reject,produced,broken,stepper,pastValue,finalizer};if(pastValue!==null&&pastValue!==undefined&&isFirstNode){stream.resolve(pastValue);stream.produced=true;// ensureNext(stream)
}return stream}/**
 * @function
 * @template T
...
// stream.next.catch(() => { })
stream.reject(reason);// stream.rejectNext(reason)
stream.broken=true;stream.stepper=null;stream.pastValue=null}}const current=yield stream.current;const next=yield stream.next;// const next = stream.nextPoint || ensureNext(stream)
return{current,next}});return function pull(_x3){return _ref2.apply(this,arguments)}})();/**
 * @function
 * @template T
 * @param {SporadicStream<T>} stream
 ...
 * @param {T} value
 * @returns {Promise<SporadicStream<T>>}
 */const push=(()=>{var _ref5=_asyncToGenerator(function*(stream,value){var _ref6=yield available(stream);const point=_ref6.point;// point.pastValue = value
point.resolve(value);point.produced=true;const result=yield point.next;// creates a new stream point/node
return result;// return ensureNext(point)
});return function push(_x5,_x6){return _ref5.apply(this,arguments)}})();/**
 * @function
 * @template T
 * @param {SporadicStream<T>} stream
...
}else{// const reason = error()
// NOTE: shallow/ignore error/reason
// point.current.catch(() => { })
// point.next.catch(() => { })
point.reject(error());// point.rejectNext(reason)
point.produced=true;point.broken=true;point.stepper=null;try{if(point.finalizer){point.finalizer()}}catch(reason){// shallow/ignore error/reason
