...
const react=async(stream,procedure)=>{const deferred=utils.defer();let currentStream=stream;try{while(true){const result=await pull(currentStream);currentStream=result.next;try{procedure(result.current)}catch(reason){deferred.reject(reason);throw reason;// next catch won't resolve deferred, resolve is ignored
...
};module.exports.open=open;module.exports.push=push;module.exports.pull=pull;module.exports.close=close;module.exports.react=react;module.exports.filter=filter;module.exports.map=map;module.exports.every=every;